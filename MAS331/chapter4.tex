...
\pf{Proof}{
    ...
    \begin{itemize}[nolistsep]
        ...
        \ii Suppose $U_p$ is defined for each $p \in P_n$ where $n > 1$, and
            $\forall p, q \in P_n,\: (p < q \implies \cl{U_p} \subseteq U_q)$ (*).        
    ...
    \[
        \forall p, q \in P,\: \big(p < q \implies \cl{U_p} \subseteq U_q\big)
        \text{.} \qquad(\ast)
    \]
...
\dfn{Parition of Unity}{
    ...
    $\{U_i\}_{i \in [n]}$ if
    \begin{itemize}[nolistsep]
        \ii $\forall i \in [n],\: (\text{support of } \phi_i) \subseteq U_i$ and
        \ii $\forall x \in X,\: \sum_{i=1}^{n} \phi_i(x) = 1$.
    \end{itemize}
}
...
\pf{Proof}{
    ...
    \begin{minipage}{.9\linewidth}
        ...
        $X$ such that $\forall i \in [n],\: \cl{V_i} \subseteq U_i$.
        \pf{Proof}{
