...
\nt{
	\begin{itemize}[nolistsep]
        ...
        \ii $[n] \coloneqq \{1, 2, \cdots, n\}$ where $n \in \ZZ_+$.
        \ii $B^A \coloneqq A \to B = \{ f \colon A \to B \}$.
            (The set of functions from $A$ to $B$.)
            $(f \colon A \to B) \coloneqq (f \in A \to B)$.
	\end{itemize}
}
...
\exmp{Partition}{
	If there are equivalence classes $E$ and $E'$, then they are either $E = E'$ or $E \cap E' = \OO$.
	This implies, if we let $\mcal{E} \coloneqq \{ E \mid E \text{ is an equivalence class of } x \text{ where } x \in A \}$,
	$A = \bigcup_{E \in \mcal{E}} E$.
}
\solve{
	Since if $E \cap E' = \OO$ it is done, suppose $E \cap E' \neq \OO$. There are $a$ and $a'$ such that
...
\dfn{Boundedness} {
	...
	\begin{itemize}[noitemsep]
		\ii $A_0$ is \textit{bounded above} by $b \in A$ if $x \in A_0 \implies x \le b$.
		      \begin{itemize}[nolistsep]
			      \ii The smallest such $b$ is called the \textit{least upper bound} or the \textit{supremum} of $A_0$.
		      \end{itemize}
...
\pf{Proof}{
	...
	using induction. For the base case, it is known the only nonempty subset of $[1]$, $\{1\}$, has $1$ as its smallest element.
...
\dfn{Finite Set and Cardinality}{
	A set $A$ is \textit{finite} if there is a bijective $f \colon A \to [n]$ for some $n \in \ZZ_+$ or $A = \OO$.
	\begin{itemize}[nolistsep]
...
\pf{Proof by Induction}{
	...
	WTS for the case $|A| = n+1$. Suppose $B \neq \OO$ because we have nothing to talk about then.
	Let $a_0 \in B$.
...
\pf{Proof}{
	...
	Since $f$ is surjective, $f^{-1}(\{a\}) \neq \OO$ for every $a \in A$.
	Let $M \coloneqq \max \{\, \min f^{-1}(\{a\}) \mid a \in A \,\}$.
	$M$ is well defined thanks to \Cref{th:wop} and the fact that $\OO \neq f^{-1}(\{a\}) \subseteq [m]$.
...
\pf{Proof}{
	...
	Moreover, $h$ is injective since it is $h(m) < h(n)$ whenever $m < n$.
...
\pf{Proof}{
	...
	$g$ is well-defined because $f^{-1}(\{a\}) \neq \OO$ for every $a \in A$ and \Cref{th:wop} holds.
	$g$ is also injective since $f^{-1}(\{a_1\}) \cap f^{-1}(\{a_2\}) = \OO$ if $a_1 \neq a_2 \in A$.
...
\solve{
    ...
    \] $f$ is naturally surjective by the construction. Therefore, $\bigcup_{i \in J} A_i$ is countable.
}
...
\solve{
    ...
    Now, construct a function $f \colon \prod_{i=1}^n A_i \to \ZZ_+$ by \[
...
\pf{Proof}{
    ...
    Let $c \colon \mcal{P}(A) \setminus \{\OO\} \to A$ be a function
    such that $c(A') \in A'$ for every $\OO \neq A' \subseteq A$.
    Its existence is guaranteed by \Cref{lem:choiceFtnExs}.
	\begin{enumerate}[nolistsep, label=(\arabic*)]
		\ii $f(1) \coloneqq c(A)$
        \ii $f(n+1) \coloneqq c(A \setminus f([n]))$ for each $n \in \ZZ_+$.
	\end{enumerate}
	\par Suppose $A \setminus f([n]) = \OO$ for some $n \in \ZZ_+$.
	...
	((ii) $\to$ (iii))
	Let $f \colon \ZZ_+ \hookrightarrow A$ be an injection.
