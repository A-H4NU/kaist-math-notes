...
\dfn{Injectivity, Surjectivity and Bijectivity}{
	...
	\begin{enumerate}[noitemsep,label=\roman*)]
		\ii \textit{injective} (or \textit{one-to-one}, 1-1) if $\forall a, a' \in A,\: f(a) = f(a') \implies a = a'$,
		\ii \textit{surjective} (or \textit{onto}) if $\forall b \in B,\: \exists a \in A,\: b = f(a)$, and
...
\exmp[compFandFinvAndSurjInj]{}{
	\begin{enumerate}[nolistsep, label=\alph*)]
		...
		\ii $f$ is injective if and only if $\forall A_0 \subseteq A,\: A_0 = f^{-1}\big(f(A_0)\big)$.
		\ii $f\big(f^{-1}(B_0)\big) \subseteq B_0$
		\ii $f$ is surjective if and only if $\forall B_0 \subseteq B,\: B_0 = f \big(f^{-1}(B_0)\big)$.
	\end{enumerate}
}
\solve{
	\paragraph*{a)}
