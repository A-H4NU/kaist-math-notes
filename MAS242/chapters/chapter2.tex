...
\thm[]{Inverse Function Theorem}{
    Let \(\vec f \colon U (\subseteq \RR[n]) \to \RR[n]\) be \(C^1\) in \(U\),
    \(\vec a \in U\), and \(\vec b = f(a)\).
...
\pf{Proof}{
    ...
    for each \(x \in B_{\delta}(\vec c)\).

    Hence, for all \(x \in B_{\delta}(\vec c)\),
    \[
    ...
    Now, fix any \(\vec y \in \RR[n]\).
    Fix \(\vec x_1, \vec x_2 \in B_{\delta}(\vec c)\). Define \(\Psi \colon [0, 1] \to \RR\) by
    ...
    \[
        ...
        = |\d \Psi(t_\ast \vec x_1 + (1-t_\ast)\vec x_2)| \cdot |(\vec x_1 - \vec x_2)|
        < |\vec x_1 - \vec x_2| / 2.
    \]
}
