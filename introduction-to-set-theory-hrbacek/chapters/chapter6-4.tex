...
\thm[transRecursion]{The Transfinite Recursion Theorem}{
    ...
    such that \(\mbf{F}(\alpha) = \mbf{G} \left(\restr{\mbf{F}}{\alpha}\right)\)
    for all ordinals \(\alpha\) where \(\restr{\mbf{F}}{\alpha}\) is the transfinite sequence
    ...
    \begin{equation}
        \begin{aligned}
            ...
            \begin{aligned}
                & \begin{aligned}
                    &x\text{ is an ordinal and }y = t(x)\text{ for some computation }t \\
                    &\qquad\text{ of length }x\text{ based on }\mbf{G},
                \end{aligned} \\
                \text{or }& x\text{ is not an ordinal and }y = \OO
            \end{aligned}
            \right.
        \end{aligned}
...
\thm[transRecursionParam]{The Transfinite Recursion Theorem: Parametric Version}{
    ...
    such that \(\mbf{F}(z, \alpha) = \mbf{G}\left(z, \restr{\mbf{F}_z}{\alpha}\right)\).
...
\pf{\hypertarget{yinFUHIP}{Proof} of {\rm\nameref{th:operationRecursion}}}{
    Define an operation \(\mbf{G}'\) by
    \[
    ...
    there exists an operation \(\mbf{F}\) such that
    \(\mbf{F}(0) = a\) and \(\mbf{F}(n + 1) = \mbf{G}(\mbf{F}(n), n)\)
    for all \(n \in \NN\).
    Then, \(\restr{\mbf{F}}{\omega}\) is the sequence we are looking for.
}
