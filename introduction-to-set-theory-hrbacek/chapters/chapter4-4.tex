...
\mlemma[orderOnUnion]{}{
    ...
    \]
    is a total ordering.
    The totally ordered set \((A, \le)\) is called the
    \textit{sum} of the totally ordered sets \((A_1, \le_1)\) and \((A_2, \le_2)\).
}
\pf{Proof}{
...
\mlemma[orderOnProduct]{}{
    ...
    \]
    is a total ordering.
    We call \(\le\) the \textit{lexicographic ordering} (\textit{lexicographic product})
    of \(A_1 \times A_2\).
}
\mclm{Proof}{\hfill
...
\thm[countImbed]{}{
    Let \((P, \preceq)\) be a countably infinite totally ordered set,
    and let \((Q, \le)\) be a countably infinite dense totally ordered set without endpoints.
    Then, there exists \(h \colon P \hookrightarrow Q\) such that
    \[\fall p, p' \in P,\: (p \prec p' \implies h(p) < h(p')).\]
}
\pf{Proof}{
    This is essentially the one-sided version of \Cref{th:countDenseTotal}.
    Let \(\lang\,p_n \mid n \in \NN\,\rang\) be an injective sequence onto \(P\).
    In a similar way as \Cref{clm:RWmAyDPu} in the proof of \Cref{th:countDenseTotal},
    ...
    The rest is the same as the proof of \Cref{th:countDenseTotal}.
}
