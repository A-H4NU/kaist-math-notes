...
\nt{
    Note that $S \cap N'(\vec c; \delta) = \OO$ for sufficiently small $\delta$
...
\pf{Proof}{
    ...
    $\fall k \in \NN,\: (k \ge k_0 \implies \vec x_k \in N(\vec c; \delta))$.
    Since $\vec x_k \neq \vec c$ for each $k \in \NN$, we may say \[
        \fall k \in \NN,\: \big( k \ge k_0 \implies \vec x_k \in N'(\vec c; \delta)
...
\pf{Proof}{
    ...
    then $\exs k_0 \in \NN,\: \fall k \in \NN_{\ge k_0},\: \vec x_k = \vec c$;
...
\thm[contiSendsConToCon]{}{
    If $S$ is a connected subset of $\RR[n]$ and $f \colon S \to \RR$ is continuous on $S$,
...
\pf{Proof}{
    ...
    Then, $S \subseteq U_1 \cup V_1 = S$, $U_1 \cap V_1 = \OO$,
    $S \cap U_1 \neq \OO$, and $S \cap V_1 \neq \OO$,
    which contradicts $S$ is connected, \#.
}

\thm[contiSendsCompToComp]{}{
    If $S$ is a compact subset of $\RR[n]$ and $f \colon S \to \RR$ is continuous on $S$,
...
\thm[contiBijOnCompSetHasContiBijInv]{}{
...
\thm[limAlgebraCS]{}{
    ...
    \begin{enumerate}[nolistsep, label=(\roman*)]
        ...
        \ii $f_1f_2 \in C(S)$.
        \ii $1/f_2 \in C(S)$, provided that $\fall \vec x \in S,\: f_2(\vec x) \neq 0$.
        \ii $f_1/f_2 \in C(S)$, provided that $\fall \vec x \in S,\: f_2(\vec x) \neq 0$.
    \end{enumerate}
}
\pf{Proof}{
...
\dfn{Uniform Continuity}{
    ...
    is said to be \textit{uniformly continuous on} $S$ if, \[
        ...
        \fall \vec c \in S,\: S \cap N(\vec c; \delta) \subseteq f\inv(N(f(\vec c); \veps))\text{.}
    \]
    Or, equivalently, \[
        \fall \veps \in \RR_+,\: \exs \delta \in \RR_+,\:
        \fall \vec x, \vec y \in S,\:
...
\thm[contiThenUnifContiOnComp]{}{
    Suppose that $f \colon S \to \RR$ is continuous on a compact subset $S$ of $\RR[n]$.
...
\dfn{Limit Point of a Set of Functions}{
    ...
    \[
        \fall \veps \in \RR_+,\: F \cap N'(f_0; \veps) \neq \OO\text{.}
    \]
}
...
\dfn{Convergence of a Sequence of Functions}{
    \begin{itemize}[nolistsep]
        \ii A sequence $\{f_k\}_{k \in \NN}$ in $B(S)$ is said to \textit{converge uniformly}
            ...
            \[
                \fall \veps \in \RR_+,\: \exs k_0 \in \NN,\:
                \fall k \in \NN,\: \big(k \ge k_0 \implies f_k \in N(f_0; \veps)\big)\text{.}
            \]
            We write
            \[
        ...
        \ii A sequence $\{f_k\}_{k \in \NN}$ in $B(S)$ is said to \textit{converge pointwise}
            ...
            \[
                \fall (\vec c, \veps) \in S \times \RR_+,\: \exs k_0 \in \NN,\:
                \fall k \in \NN,\: \big(k \ge k_0 \implies f_k(\vec c) \in N(f_0(\vec c); \veps)\big)\text{.}
            \]
            We write
            \[
        ...
        \ii A sequence $\{f_k\}_{k \in \NN}$ in $B(S)$ is said to be \textit{(uniformly) Cauchy} if
            \[
                \fall \veps \in \RR_+,\: \exs k_0 \in \NN,\:
                \fall k, m \in \NN,\: \big( k, m \ge k_0 \implies \|f_m - f_k\|_\infty < \veps \big)\text{.}
            \]
    \end{itemize}
}
...
\thm[cauchyBddFtn]{}{
    ...
    That is, $\exs M \in \RR_+,\: \fall k \in \NN,\: \|f_k\|_\infty \le M$.
}
\pf{Proof}{
...
\pf{Proof}{
    ...
    Since $\{f_k\}$ is Cauchy,
    \[
        \exs k_0 \in \NN,\: \fall k, m \in \NN,\: \big(k, m \ge k_0 \implies \|f_k - f_m\|_\infty < \veps/2\big)\text{.}
    \]
    Then, for each $k \in \NN_{\ge k_0}$,
    \[
        \fall \vec c \in S,\: |f_k(\vec c) - f_0(\vec c)| = \lim_{m \to \infty} |f_k(\vec c) - f_m(\vec c)| \le \veps/2\text{.}
    \]
    ...
    \[
        \exs k_0 \in \NN,\: \fall k \in \NN,\: \big(k \ge k_0 \implies \|f_k - f_0\|_\infty < 1\big)\text{.}
    \]
    This implies $\fall \vec c \in S,\: |f_{k_0}(\vec c) - f_0(\vec c)| < 1$.
    Hence, $\fall \vec c \in S,\: |f_0(\vec c)| < |f_{k_0}(\vec c)| + 1 \le \|f_{k_0}\|_\infty + 1$.
...
\nt{
    If $S \neq \OO$ is a compact subset of $\RR$,
...
\pf{Proof}{
    ...
    Therefore,
    \[
        \exs \delta \in \RR_+, \fall x, y \in [0, 1],\:
...
\section{Vector-Valued Functions on $\mathbb R^n$}

\dfn{Component Function}{
    Let $\vec f$ be a function with domain $S \subseteq \RR[n]$ and codomain $T \subseteq \RR[m]$.
    For $\vec x \in S$, we write
    \[
        \vec f(\vec x) = \vec y = (y_1, y_2, \cdots, y_m)\text{.}
    \]
    Then, for each $j \in [m]$, there is a real-valued function $f_i \colon S \to \RR$ defined by
    $f_j(\vec x) = y_j$.
    The functions $f_1, f_2, \cdots, f_m$ are called \textit{component functions} of $\vec f$.
    We write \[
        \vec f = (f_1, f_2, \cdots, f_m)\text{.}
    \]
}

\dfn{Limit and Continuity of Vector-Valued Functions}{
    Let $\OO \neq S \subseteq \RR[n]$ and $\vec f \colon S \to \RR[m]$.
    \begin{itemize}[nolistsep]
        \ii Let $\vec c \in \cl S$.
            We say that $\vec f$ has \textit{limit} $\vec v$ as $\vec x$ approaches $\vec c$,
            and we write \[
                \lim_{\vec x \to \vec c} \vec f(\vec x) = \vec v
            \] if,
            for every neighborhood $N(\vec v)$, there exists a deleted neighborhood $N'(\vec c)$
            such that \[
                S \cap N'(\vec c) \subseteq \vec f\inv(N(\vec v))\text{.}
            \]
        \ii Let $\vec c \in S$.
            We say that $\vec f$ is \textit{continuous at} $\vec c$ if \[
                \lim_{\vec x \to \vec c} \vec f(\vec x) = \vec f(\vec c)\text{.}
            \]
        \ii We say that $\vec f$ is \textit{continuous on} $S$ if
            $\vec f$ is contiuous at every point of $S$.
    \end{itemize}
}

\thm[vecLimAndComponentFtns]{}{
    Let $\OO \neq S \subseteq \RR[n]$,
    $\vec f \colon S \to \RR[m]$, and $\vec f = (f_1, f_2, \cdots, f_m)$.
    \begin{enumerate}[nolistsep, label=(\roman*)]
        \ii Let $\vec c \in \cl S$.
            Then $\lim_{\vec x \to \vec c} \vec f(\vec x) = \vec v = (v_1, v_2, \cdots, v_m)$
            if and only if, for each $j \in [m]$, $\lim_{\vec x \to \vec c} f_j(\vec x) = v_j$.
        \ii Let $\vec c \in S$.
            Then $\vec f$ is continuous at $\vec c$ if and only if,
            for each $j \in [m]$, $f_j$ is continuous at $\vec c$.
        \ii $\vec f$ is continuous on $S$ if and only if,
            for each $j \in [m]$, $f_j$ is continuous on $S$.
    \end{enumerate}
}
\pf{Proof}{
    (ii) and (iii) directly follows from (i). So, we only need to prove (i).

    ($\Rightarrow$) Take any $\veps \in \RR_+$.
    Then, there exists $\delta \in \RR_+$ such that
    \[
        \fall \vec x \in S,\:
        \big(0 < \|\vec x - \vec c\| < \delta \implies \|\vec f(\vec x) - \vec v\| < \veps\big)\text{.}
    \]
    Then, for each $j \in [m]$,
    whenever $\vec x \in S \cap N'(\vec c; \delta)$,
    \[
        |f_j(\vec x) - v_j|^2 \le \sum_{i=1}^{m} |f_j(\vec x)-v_i|^2 = \|\vec f(\vec x) - \vec v\|^2 < \veps^2\text{,}
    \]
    which implies $S \cap N'(\vec c; \delta) \subseteq f_j\inv(N(v_j; \veps))$;
    $\lim_{\vec x \to \vec c} f_j(\vec x) = v_j$.

    ($\Leftarrow$) Take any $\veps \in \RR_+$.
    Then, for each $j \in [m]$, there exists $\delta_j \in \RR_+$ such that
    \[
        \fall \vec x \in S,\:
        \big( 0 < \|\vec x - \vec c\| < \delta_j \implies |f_j(\vec x) - v_j| < \veps / \sqrt{m} \big)\text{.}
    \]
    Then, whenever $\vec x \in S \cap N'(\vec c; \delta_0)$
    where $\delta_0 \triangleq\min_{j=1}^m \delta_j$,
    \[
        \|\vec f(\vec x) - \vec v\|^2 = \sum_{i=1}^{m} |f_j(\vec x) - v_j|^2
        < \sum_{i=1}^{m} \left(\frac{\veps}{\sqrt{m}}\right)^2 = \veps^2\text{,}
    \]
    which implies $S \cap N'(\vec c; \delta_0) \subseteq \vec f\inv(N(\vec v; \veps))$;
    $\lim_{\vec x \to \vec c} \vec f(\vec x) = \vec v$.
}

\thm[vecLimIffEveryCauchyConv]{}{
    Let $\OO \neq S \subseteq \RR[n]$ and $\vec f \colon S \to \RR[m]$.

    \begin{enumerate}[nolistsep, label=(\roman*)]
        \ii Let $\vec c$ be a point in $\cl S$.
            Then $\lim_{\vec x \to \vec c} \vec f(\vec x) = \vec v$
            if and only if, for every sequence $\{\vec x_k\}$ in $S \setminus \{\vec c\}$ such that
            $\lim_{k \to \infty} \vec x_k = \vec c$,
            we have $\lim_{k \to \infty} \vec f(\vec x_k) = \vec v$.
        \ii Let $\vec c$ be a point in $S$.
            Then $\vec f$ is continuous at $\vec c$
            if and only if, for every sequence $\{\vec x_k\}$ in $S$ such that
            $\lim_{k \to \infty} \vec x_k = \vec c$,
            we have $\lim_{k \to \infty} \vec f(\vec x_k) = \vec f(\vec c)$.
    \end{enumerate}
}
\pf{Proof}{
    By \Cref{th:vecLimAndComponentFtns},
    \[
        \lim_{\vec x \to \vec c} \vec f(\vec x) = \vec v
        \iff \fall j \in [m], \lim_{\vec x \to \vec c} f_j(\vec x) = v_j
    \]
    where $\vec v = (v_1, v_2, \cdots, v_m)$.
    By \Cref{th:limIffEveryCauchyConv}, for each $j \in [m]$,
    \[
        \lim_{\vec x \to \vec c} f_j(\vec x) = v_j
        \iff \fall \{\vec x_k\} \in (S \setminus \{\vec c\})^\omega,\:
        \big(\lim_{k \to \infty} \vec x_k = \vec c
             \implies \lim_{k \to \infty} f_j(\vec x_k) = v_j\big)\text{.}
    \]
    By \Cref{th:seqConvIffCoordsConv},
    \[
        \lim_{k \to \infty} \vec f(\vec x_k) = \vec v \iff
        \fall j \in [m], \lim_{k \to \infty} f_j(\vec x_k) = v_j\text{.}
    \]
    Thus, (i) is proven,
    and (ii) can be proven similarly with the aid of \Cref{th:contiIffEveryCauchyConv}.
}

\thm[vecContiTopDef]{}{
    Let $\vec \vec f \colon S \to T$ where $S \subseteq \RR[n]$ and $T \subseteq \RR[m]$.
    Suppose $f$ is surjective.
    Then $\vec f$ is continuous on $S$ if and only if the inverse image of
    every relatively open set in $T$ is relatively open in $S$.
}
\pf{Proof}{
    Repeat the proof of \Cref{th:contiTopDef} verbatim.
}

\thm[vecContiSendsConToCon]{}{
    If $S$ is a connected subset of $\RR[n]$ and $\vec f \colon S \to \RR[m]$ is continuous on $S$,
    then $T = \vec f(S)$ is also connected.
}
\pf{Proof}{
    Repeat the proof of \Cref{th:contiSendsConToCon} verbatim.
}

\thm[vecContiSendsCompToComp]{}{
    If $S$ is a compact subset of $\RR[n]$ and $\vec f \colon S \to \RR[m]$ is continuous on $S$,
    then $T = \vec f(S)$ is also compact.
}
\pf{Proof}{
    Repeat the proof of \Cref{th:contiSendsCompToComp} verbatim.
}

\thm[vecCompOfContiIsContiPoint]{}{
    Let $S \subseteq \RR[n]$, $f \colon S \to \RR[m]$, $f(S) \subseteq T \subseteq \RR[m]$,
    and $g \colon T \to \RR[p]$.
    If $f$ is continuous at $\vec c \in S$ and if $g$ is continuous at $f(\vec c) \in T$,
    then $g \circ f$ is continuous at $\vec c$.
}
\pf{Proof}{
    Repeat the proof of \Cref{th:compOfContiIsContiPoint} verbatim.
}

\thm[vecContiBijOnCompSetHasContiBijInv]{}{
    Let $\OO \neq S \subseteq \RR[n]$ is compact and $\OO \neq T \subseteq \RR[m]$.
    Let $\vec f \colon S \to T$ is continuous on $S$ and bijective.
    Then, $\vec f\inv$ is also continuous on $\vec f(S)$.
}
\pf{Proof}{
    Repeat the proof of \Cref{th:contiBijOnCompSetHasContiBijInv} verbatim.
}

\dfn{Uniform Continuity}{
    A function $\vec f \colon S \to \RR[m]$ with $S \subseteq \RR[n]$
    is said to be \textit{uniformly continuous on} $S$ if, \[
        \fall \veps \in \RR_+,\: \exs \delta \in \RR_+,\:
        \fall \vec c \in S,\: S \cap N(\vec c; \delta) \subseteq \vec f\inv(N(\vec f(\vec c); \veps))\text{.}
    \]
    Or, equivalently, \[
        \fall \veps \in \RR_+,\: \exs \delta \in \RR_+,\:
        \fall \vec x, \vec y \in S,\:
        \big( \|\vec x-\vec y\| < \delta \implies \|\vec f(x)-\vec f(y)\| < \veps \big)\text{.}
    \]
}

\thm[]{}{
    Let $\vec f = (f_1, f_2, \cdots, f_m)$ be a function $\vec f \colon S \to
    \RR[m]$ where $S \subseteq \RR[n]$. Then, $\vec f$ is uniformly continuous
    if and only if $f_j$ is uniformly continuous on $S$ for each $j \in [m]$.
}
\pf{Proof}{
    ($\Rightarrow$)
    Take any $j \in [m]$ and $\veps \in \RR_+$. Then, there exist $\delta \in
    \RR_+$ such that, \[
        \fall \vec x, \vec y \in S,\: \big(\|\vec x-\vec y\| < \delta
        \implies \|\vec f(\vec x) - \vec f(\vec y)\| < \veps\big)\text{.}
    \]
    Since $|f_j(\vec x) - f_j(\vec y)| \le \|\vec f(\vec x) - \vec f(\vec y)\|$,
    $f_j$ is uniformly continuous.

    ($\Leftarrow$)
    Take any $\veps \in \RR_+$. Then, for each $j \in [m]$, there exists
    $\delta_j \in \RR_+$ such that \[
        \fall \vec x, \vec y \in S,\: \big(\|\vec x-\vec y\| < \delta_j
        \implies |f_j(\vec x) - f_j(\vec y)| < \veps/\sqrt{m}\big)\text{.}
    \]
    Let $\delta \triangleq \min_{j \in [m]} \delta_j$. Then,
    \[
        \fall \vec x, \vec y \in S,\: \big(\|\vec x-\vec y\| < \delta
        \implies \|\vec f(\vec x) - \vec f(\vec y)\| < \veps\big)\text{.}
    \]
...
\thm[vecContiThenUnifContiOnComp]{}{
    Suppose that $f \colon S \to \RR[m]$ is continuous on a compact subset $S$ of $\RR[n]$.
    Then $f$ is uniformly continuous on $S$.
}
\pf{Proof}{
    Repeat the proof of \Cref{th:contiThenUnifContiOnComp} verbatim.
}
