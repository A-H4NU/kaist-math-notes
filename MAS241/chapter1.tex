...
\dfn{Boundedness}{
	...
	\begin{enumerate}[noitemsep,label=\alph*)]
		\ii $S$ is \textit{bounded above} if $\exists M \in \RR,\: \fall x \in S,\: x \le M$.
		      \begin{itemize}
			      \ii $M$ is called an \textit{upper bound} of $S$.
		      \end{itemize}
		\ii $S$ is \textit{bounded below} if $\exists M \in \RR,\: \fall x \in S,\: x \ge M$.
		      \begin{itemize}
...
\thm[archi]{Archimedes' Principle}{
	Let $\veps$ and $M$ be any two positive real numbers.
...
\thm[sup1]{}{
	...
	\begin{itemize}
		\ii If $S$ is bounded above, then ``$\mu = \sup S$ if and only if
		      $\mu \text{ is an upper bound and }\fall\: \veps \in \RR_+,\: \exists\: x \in S,\: \mu-\veps < x \le \mu$''.
		\ii If $S$ is bounded below, then ``$\nu = \inf S$ if and only if
		      $\nu \text{ is an lower bound and }\fall\: \veps \in \RR_+,\: \exists\: x \in S,\: \nu \le x < \nu+\veps$''.
	\end{itemize}
}
\pf{Proof}{
...
\thm[archi2]{}{
	\Cref{th:archi} (Archimedes' principle) is equivalent to the following statement: \[
		\fall\, c \in \RR_+,\: \exists\: k \in \NN,\: k-1 \le c < k\text{.}
	\]
}
\pf{Proof}{
	...
	Now, assume ``$\fall\: c \in \RR_+,\: \exists\: k \in \NN,\: k-1 \le c < k$''.
...
\pf{Proof}{
	...
	By \Cref{th:archi2}, $\exists\: p \in \NN,\: p - 1 \le cq < p$ since $cq > 0$.
...
\dfn{Limit Point and Isolated Point}{
	For $\OO \neq S \subseteq \RR$, $x \in \RR$ is a \textit{limit point} of $S$ if \[
		\fall \veps \in \RR_+,\: N'(x, \veps) \cap S \neq \OO\text{.}
	\]
...
\thm[limPointWeak]{}{
	Let $\OO \neq S \subseteq \RR$. Then, $x \in \RR$ is a limit point of $S$ if and only if \[
		\exists \veps_0 \in \RR_+,\: \fall \veps \in (0, \veps_0),\: N'(x, \veps) \cap S \neq \OO\text{.}
	\]
}
...
\pf{Proof}{
	...
	If we let $\veps_0 \coloneqq |x-x_j|/2$, $N'(x; \veps_0) \cap S = \OO$, \#.
}
...
\begin{enumerate}[nolistsep, label=(\alph*)]
	...
	\ii $\fall k \in \NN,\: b_k - a_k = (b_0 - a_0)/2^k > 0$, and
	\ii $\fall k \in \NN,\: a_{k-1} \le a_{k} < b_{k} \le b_{k-1}$.
\end{enumerate}
...
Since $a_j$ is a lower bound of $\{b_k\}_{k=0}^\infty$ for all $j \in \NN$, $\fall j \in \NN,\: a_j \le \beta$.
...
\dfn[cluster]{Cluster Point}{
	$c \in \RR$ is a \textit{cluster point} of the sequence $\{x_k\}$ if, \[
		\fall (\veps, k) \in \RR_+ \times \NN,\: \exists k_1 \in \NN_{>k},\: x_{k_1} \in N(c; \veps)\text{.}
	\]
}
...
\dfn{Convergence and Divergence of a Sequnce}{
	The sequnce $\{x_k\}$ \textit{converges} to $x_0$ and $x_0$ is the \textit{limit} of $\{x_k\}$ if, \[
		\fall \veps \in \RR_+,\: \exists k_0 \in \NN,\: \fall k \in \NN_{\ge k_0},\: x_k \in N(x_0; \veps)\text{.}
...
\mlemma{}{
	...
	\begin{enumerate}[nolistsep, label=(\roman*)]
		...
		\ii $\lim_{k \to \infty} x_k = x_0 \implies \fall c \in \RR,\: \lim_{k \to \infty} cx_k = cx_0$
	\end{enumerate}
}
\pf{Proof of (ii)}{
...
\pf{Proof}{
	Take any $\veps > 0$.
	There is $k_1 \in \NN$ such that $\fall k \in \NN_{\ge k_1},\: x_k \in N(L; \veps)$.
	Similarly, there is $k_2 \in \NN$ such that $\fall k \in \NN_{\ge k_2},\: x_k \in N(L; \veps)$.
...
Take any $\veps \in \RR_+$ and $k \in \NN$. Then, there is some $j_0 \in \NN$ such that $\fall j \in \NN_{\ge j_0},\: x_{k_j} \in N(c; \veps)$.
...
There is $i_0 \in \NN$ such that $\fall i \in \NN_{\ge i_0},\: |x_i| < \veps$.
Since $k_i \ge i$ for all natural number $i$, $\fall i \in \NN_{\ge i_0},\: |x_{k_i}| < \veps$.
...
\dfn{Limit Superior and Inferior}{
	...
	\begin{itemize}
		\ii $\limsup x_k \triangleq \begin{cases}
				      ...
				      -\infty & \text{if } \{x_k\} \text{ is bounded above but unbounded below and } C = \OO    \\
		...
		\ii $\liminf x_k \triangleq \begin{cases}
				      ...
				      \infty  & \text{if } \{x_k\} \text{ is bounded below but unbounded above and } C = \OO    \\
...
\thm[bddSeqLimSup]{}{
	...
	\begin{itemize}[nolistsep]
		\ii Let $\mu = \limsup x_k$.
		      \begin{itemize}[nolistsep]
			      \ii $\exs k_0 \in \NN,\: \fall k \in \NN_{\ge k_0},\: x_k < \mu + \veps$.
			      \ii $\fall k \in \NN,\: \exs k_1 \in \NN_{>k},\: x_{k_1} > \mu - \veps$.
		      \end{itemize}
		\ii Let $\nu = \liminf x_k$.
		      \begin{itemize}[nolistsep]
			      \ii $\exs k_0 \in \NN,\: \fall k \in \NN_{\ge k_0},\: x_k > \nu - \veps$.
			      \ii $\fall k \in \NN,\: \exs k_1 \in \NN_{>k},\: x_{k_1} < \nu + \veps$.
		      \end{itemize}
	\end{itemize}
}
\pf{Proof}{
	...
	$\fall k \in \NN,\: \exs k_1 \in \NN_{>k},\: x_{k_1} > \mu - \veps$. (See \Cref{lem:clusterSetInf}.)
...
\pf{Proof}{
	...
	\begin{enumerate}[label=(\roman*), nolistsep]
		\ii ($\Rightarrow$) $C = \{x_0\}$, therefore $\liminf x_k = \limsup x_k = x_0$. \\
		      ...
		      There are natural numbers $k_1$ and $k_2$ such that $\fall k \in \NN_{\ge k_1},\: x_k < x_0 + \veps$
		      and $\fall k \in \NN_{\ge k_0},\: x_k > x_0 - \veps$.
...
Now, we claim that \[
	\fall \veps \in \RR_+,\: \exs k_0 \in \NN,\: \fall\: k \in \NN_{\ge k_0},\: z_k > L - \veps\text{.}
\]
...
Then, for all $k \in \NN_{\ge k_1}$, $x_k > (L - \veps/2)^{k-k_1}x_{k_1}$, which is equivalent to \[
...
$\exs k_3 \in \NN,\: \fall k \in \NN_{\ge k_3},\: z_k > L - \veps_0 = \left(L + \liminf z_k\right)/2$.

Nevertheless, by \Cref{th:bddSeqLimSup}, there is some $k_4 \in \NN_{> k_3}$ such that
$z_{k_4} < \liminf z_k + \veps_0 = \left(L + \liminf z_k\right)/2$, which is a contradiction.
...
\dfn{Cauchy Seqeunce}{
	A seqeunce $\{x_k\}$ in $\RR$ is called a \textit{Cauchy sequence} if \[
		\fall \veps \in \RR_+,\: \exs k_0 \in \NN,\: \fall k, m \in \NN_{\ge k_0},\: |x_k-x_m| < \veps\text{.}
	\]
}
\thm[convThenCauchy]{}{
...
Choose $k_0 \in \NN$ such that $\fall k, m \in \NN_{\ge k_0},\: |x_k-x_m| < \veps$.
...
A sequence $\{x_k\}$ is said to be \textit{contractive} if there exists a constant $C$, with $0 < C < 1$, such that \[
	\fall k \in \NN_{>1},\: |x_{k+1} - x_k| \le C |x_k - x_{k-1}|\text{.}
\]
}
...
\pf{Proof}{
Suppose $0 < C < 1$ and $\fall k \in \NN_{>1},\: |x_{k+1} - x_k| \le C|x_k - s_{k-1}|$.
If it is trivial when $|x_2 - x_1| = 0$, so supose $|x_2 - x_1| \neq 0$.
By induction, $\fall k \in \NN_{>1},\: |x_{k+1} - x_k| \le C^{k-1}|x_2-x_1|$.

To prove $\{x_k\}$ is a Cauchy sequence, take any $\veps \in \RR_+$.
Since $\lim_{k \to \infty} C^{k-1} = 0$, \[
	\exs k_0 \in \NN,\: \fall k \in \NN_{\ge k_0},\: C^{k-1} < \dfrac{(1-C)\veps}{|x_2-x_1|}\text{.}
\]
